...
function filterSelection(c) {
  ...
  Array.prototype.forEach.call(filterDivs, function(mydiv) {
    myRemoveClass(mydiv, "show");
    if (mydiv.className.indexOf(c) > -1) myAddClass(mydiv, "show");
});
}
...
function myAddClass(element, name) {
  ...
  arr2 = name.split(" ");
  Array.prototype.forEach.call(arr2, function(myel) {
    if (arr1.indexOf(myel) == -1) {element.className += " " + myel;}
  });
}

function myRemoveClass(myElem, myName) {
  var arr11 = myElem.className.split(" ");
  var arr22 = myName.split(" ");
  Array.prototype.forEach.call(arr22, function(myel) {
    while (arr11.indexOf(myel) > -1) {
      arr11.splice(arr11.indexOf(myel), 1);     
    }
  });
  myElem.className = arr11.join(" ");
}
